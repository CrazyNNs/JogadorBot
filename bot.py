...
_con_setup = _sqlite3_connect_original("jogadorbot.db", timeout=15)
...
RACAS_POR_ESPECIE = {
    "Cachorros": [
        ...
        {"nome": "Labrador", "emoji": "🐕", "preco": 20000, "moeda": "joyens", "raridade": "Comum",
         "descricao": "Amigável, brincalhão e extremamente leal ao dono."},
        {"nome": "Pastor Alemão", "emoji": "🐕", "preco": 28000, "moeda": "joyens", "raridade": "Raro",
         "descricao": "Protetor e inteligente, um dos mais fáceis de adestrar."},
        {"nome": "Husky Siberiano", "emoji": "🐺", "preco": 35000, "moeda": "joyens", "raridade": "Raro",
         "descricao": "Cheio de energia, ama aventuras e longas caminhadas."},
        {"nome": "Doberman", "emoji": "🐕‍🦺", "preco": 55000, "moeda": "joyens", "raridade": "Épico",
         "descricao": "Vigilante e veloz, parece estar sempre atento aos perigos das minas."},
        {"nome": "Logo Glacial", "emoji": "🐺", "preco": 110000, "moeda": "joyens", "raridade": "Lendário",
         "descricao": "Dizem que nasceu nas regiões congeladas mais profundas do deserto ártico."},
    ...
    "Gatos": [
        ...
        {"nome": "Gato-de-Cristal", "emoji": "🐈", "preco": 65000, "moeda": "joyens", "raridade": "Épico",
         "descricao": "Seu pelo brilha como minério puro. Um achado raríssimo da mineração."},
        {"nome": "Gato Ignis", "emoji": "🐈", "preco": 140000, "moeda": "joyens", "raridade": "Lendário",
         "descricao": "Dizem que seu corpo se formou após uma erupção do Kīlauea no Havaí."},
...
MISSOES_SEMANAIS = [
    {
        "id": "trabalhar_semana",
        "nome": "💼 Escravo do CLT",
        "descricao": "Use !trabalhar 20 vezes nessa semana.",
    ...
    },
    {
        "id": "apostar_semana",
        "nome": "🎰 Jackpot!",
        "descricao": "Use !apostar 15 vezes nessa semana.",
    ...
    },
    {
        ...
        "meta": 7,
        "tipo_recompensa": "xp",
        "quantidade_recompensa": 1000,
    },
    {
        "id": "joyens_acumulados_semana",
        "nome": "💰 Mão de Vaca",
        "descricao": "Acumule 50.000 Joyens nessa semana.",
        "condicao": "joyens_acumulados",
        "meta": 50000,
        "tipo_recompensa": "xp",
        "quantidade_recompensa": 200,
    },
    {
        "id": "msg_semana",
        "nome": "💬 Matraca",
    ...
    },
    {
        ...
        "descricao": "Fique 2880 minutos em call nessa semana.",
...
class ViewPerfil(discord.ui.LayoutView):
    ...
    def montar(self):
        ...
        botao_banners = discord.ui.Button(label="Banners", emoji="🖼️", style=discord.ButtonStyle.primary)
...
class ViewConsumiveisMineracao(ui.LayoutView):
    def __init__(self, usuario_id, view_mineracao):
        super().__init__(timeout=120)
        self.usuario_id = usuario_id
        self.view_mineracao = view_mineracao
        self.montar()
...
class ViewMineracao(ui.LayoutView):
    def __init__(self, usuario_id, ctx):
        ...
        self.btn_consumir.callback = self.consumir_callback
    ...
    def montar(self):
        ...
        linha.add_item(self.btn_consumir)
        container.add_item(linha)
    ...
    async def consumir_callback(self, interaction: discord.Interaction):
        try:
            if interaction.user.id != self.usuario_id:
    ...
    async def aplicar_penalidade_morte(self, causa):
        ...
        container.add_item(ui.TextDisplay(
            ...
            f"Você recuperou **{hp_recuperado} de HP** ({int(hp_recuperado)}/{maximo}) após o descanso forçado."
        ))
    ...
    async def escolher_emprego(self, interaction: discord.Interaction, emprego_nome):
        ...
        try:
            ...
            if atual and atual[0] == emprego_nome:
                ...
                INSERT INTO empregos_usuarios (usuario_id, emprego, vezes_trabalhadas)
                VALUES (?, ?, 0)
                ON CONFLICT(usuario_id) DO UPDATE SET emprego = ?, vezes_trabalhadas = 0
...
async def double(ctx, quantidade: int = None, cor: str = None):
    ...
    janela_final = esteira[DOUBLE_FRAMES - 1:DOUBLE_FRAMES - 1 + DOUBLE_JANELA]
...
@banner_group.command(name="dar", description="Dá um banner específico para um usuário (admin)")
@app_commands.describe(
    membro="Quem vai receber o banner",
    nome="Nome exato do banner"
)
@app_commands.check(lambda interaction: eh_admin(interaction.user.id))
async def banner_dar(interaction: discord.Interaction, membro: discord.Member, nome: str):
    if membro.bot:
        await interaction.response.send_message("<:Atencao:1534592266625093662> Não é possível dar banner para um bot!", ephemeral=True)
        return

    con = sqlite3.connect("jogadorbot.db")
    cur = con.cursor()
    cur.execute("SELECT id, nome FROM banners WHERE LOWER(nome) = LOWER(?)", (nome,))
    resultado = cur.fetchone()

    if not resultado:
        con.close()
        await interaction.response.send_message(
            f"<:Atencao:1534592266625093662> Banner **{nome}** não encontrado. Confira o nome exato no catálogo.",
            ephemeral=True
        )
        return
...
@banner_dar.error
async def banner_dar_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.CheckFailure):
        await interaction.response.send_message("<:Atencao:1534592266625093662> Você não tem permissão para usar este comando.", ephemeral=True)
    else:
        raise error
